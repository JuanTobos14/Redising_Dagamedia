import React from 'react';
import './Footer.css';
// 1. Importamos la imagen del logo desde la carpeta assets
import logoDaga from '../../assets/Logo-DAGAMEDIA-Encabezado.png';

function Footer() {
  return (
    <footer className="footer-container">
      <div className="footer-content">
        
        {/* COLUMNA IZQUIERDA: Redes y Móvil */}
        <div className="footer-left">
          <div className="social-media-icons">
            {/* Facebook */}
            <a href="https://facebook.com" target="_blank" rel="noreferrer" className="social-btn facebook">
              <svg viewBox="0 0 24 24" className="icon-svg">
                <path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.95c4.56-.93 8-4.96 8-9.8z"/>
              </svg>
            </a>

            {/* Instagram */}
            <a href="https://instagram.com" target="_blank" rel="noreferrer" className="social-btn instagram">
              <svg viewBox="0 0 24 24" className="icon-svg">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.051.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/>
              </svg>
            </a>

            {/* YouTube */}
            <a href="https://youtube.com" target="_blank" rel="noreferrer" className="social-btn youtube">
              <svg viewBox="0 0 24 24" className="icon-svg">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
              </svg>
            </a>

            {/* X (Twitter) */}
            <a href="https://x.com" target="_blank" rel="noreferrer" className="social-btn x-twitter">
              <svg viewBox="0 0 24 24" className="icon-svg">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
              </svg>
            </a>
          </div>
          
          <div className="phone-info">
            <span className="phone-label">Móvil</span>
            <span className="phone-numbers">+57 3112225433 / +57 3112225455</span>
          </div>
        </div>

        {/* COLUMNA CENTRAL: Copyright */}
        <div className="footer-center">
          <p>Copyright © 2026 Dagamedia</p>
        </div>

        {/* COLUMNA DERECHA: Logo e Información Corporativa */}
        <div className="footer-right">
          {/* 2. Reemplazamos el texto e incrustamos la variable de la imagen */}
          <img src={logoDaga} alt="Logo Dagamedia" className="daga-logo-img" />
          <p className="address-text">Calle 68 6-66 Tunja – Boyacá – Colombia</p>
        </div>

      </div>
    </footer>
  );
}

export default Footer;